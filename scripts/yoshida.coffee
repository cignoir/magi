...
module.exports = (robot) ->
  robot.hear /.+/i, (msg) ->
    if (lot 10)
      ...
      robot.hear /意味/i, (msg) -> if (lot 70) then msg.send random_word ["意味としてはそのままなのです。"]
      robot.hear /(ところで|話変わる)/i, (msg) -> if (lot 70) then msg.send random_word ["話題が脱線しすぎですよね（笑）"]
      robot.hear /吉魔/i, (msg) -> if (lot 70) then msg.send random_word ["お願いだから吉魔と呼ぶのは……というのは冗談で（笑）"]
      robot.hear /ん？/i, (msg) -> if (lot 70) then msg.send random_word ["まず「ん？」とは思ってないです。"]
      robot.hear /フラッシュ/i, (msg) -> if (lot 70) then msg.send random_word ["仕様上は、フラッシュの使用はとりあえず正解なんです。"]
