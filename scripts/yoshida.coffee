...
module.exports = (robot) ->
  ...
  robot.hear /意味/i, (msg) -> if (lot 70) then msg.send ["意味としてはそのままなのです。", "はい"].sample
  robot.hear /(ところで|話変わる)/i, (msg) -> if (lot 70) then msg.send ["話題が脱線しすぎですよね（笑）", "はい"].sample
  robot.hear /ん？/i, (msg) -> if (lot 70) then msg.send ["まず「ん？」とは思ってないです。", "はい"].sample
  robot.hear /フラッシュ/i, (msg) -> if (lot 70) then msg.send ["仕様上は、フラッシュの使用はとりあえず正解なんです。", "はい"].sample
  robot.hear /スレ/i, (msg) -> if (lot 50) then msg.send ["2ちゃんねるを見ると、心が折れるので(笑)"].sample
  
  robot.hear /(お願い|頼み|出来.+？|でき.+？)/i, (msg) ->
    if (lot 50) 
      msg.send ["ムリです。死んでしまいます（笑）",
                "何とかなるよう、もがいています（笑）",
  ...
  robot.hear /(考えておきます|考えときます)/i, (msg) ->
    if (lot 50) 
      msg.send ["僕は投げられたボールは全力で打ち返す主義なので、こちらも何か考えたいですね。負けられないです(笑)", "はい"].sample

  robot.hear /(ルーレット|ルレ)/i, (msg) ->
    if (lot 50) 
      msg.send ["極まった遊びかたをされる方はコンテンツルーレットを毎日1回はやるのかなと。",
  ...
  robot.hear /デイリー/i, (msg) ->
    if (lot 50) 
      msg.send ["デイリーはべつに毎日リセットされますし、1週1回の場合も月曜日以外は、それほど焦る必要はないでしょう。",
                "理想は「気がついたら貯まっていた」です。",
  ...
  robot.hear /見え/i, (msg) ->
    if (lot 50) 
      msg.send ["見えて当然なんだよ、いいからこっちを何とかしろ！",
                "そもそもなぜ、指示した修正が入ってないの？",
                "はい"
               ].sample
